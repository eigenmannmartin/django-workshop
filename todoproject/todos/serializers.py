...
class TodoSerializer(serializers.HyperlinkedModelSerializer):
  ...
  class Meta:
    model = Todo
    fields = ['id', 'url', 'description', 'created', 'modified', 'done', 'collection']

  def get_fields(self):
      fields = super(TodoSerializer, self).get_fields()
      fields['collection'].queryset = Collection.objects.filter(owner=self.context['request'].user)
      return fields
...
class CollectionSerializer(serializers.HyperlinkedModelSerializer):
  ...
  def get_todos(self, collection):
    return TodoSerializer(collection.todos, many=True, context={"request": self.context['request']}).data
